...
function AbogadoCard({ emp, index, onClick }) {
  const [hovered, setHovered] = useState(false)
  const colors = ['#1E3A5F', '#1a3360', '#254870', '#2d4a7a', '#163050']
  const bg = colors[emp.nombre.charCodeAt(0) % colors.length]
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        fontSize: '24px', fontWeight: '600', color: '#C5A96A',
        letterSpacing: '0.03em', flexShrink: 0,
        boxShadow: '0 2px 8px rgba(20,40,69,0.18)',
      }}>
      ...
      <div>
        ...
        <p style={{ fontSize: '12px', fontWeight: '500', color: '#8A9BAD', margin: 0 }}>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
      <span style={{
        ...
        color: '#8A9BAD', letterSpacing: '0.12em',
  ...
  const cargar = async () => {
    ...
    if (user) {
      const { data: emp } = await supabase
        .from('empleados').select('nombre').eq('user_id', user.id).single()
    ...
    setEmpleados((emps || []).map(e => ({ ...e, total: counts[e.id] || 0 })))
    setLoading(false)
  }
    ...
    <div style={{ minHeight: '100vh', background: '#f7f8fa', fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <div style={{
            ...
            border: '1.5px solid #C5A96A', borderRadius: '7px',
            display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
          }}>
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '17px', fontWeight: '600', color: '#C5A96A', letterSpacing: '-0.02em' }}>
              120
            </span>
          </div>
          <span style={{ fontSize: '13px', fontWeight: '500', color: '#D6DFE8', letterSpacing: '0.01em' }}>
            Notaría Pública No. 120 · Monterrey
          </span>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {userName && (
            <span style={{ fontSize: '12px', fontWeight: '500', color: '#8A9BAD' }}>{userName}</span>
          )}
          <button
            onClick={() => navigate('/trabajos/nuevo')}
            style={{
              background: '#C5A96A', border: 'none', borderRadius: '5px',
              color: '#142845', fontSize: '11px', fontWeight: '700',
          ...
          <button
            ...
            style={{
              background: 'transparent', border: '1px solid rgba(138,155,173,0.35)', borderRadius: '5px',
              color: '#8A9BAD', fontSize: '11px', fontWeight: '500',
      ...
      <div style={{
        ...
        <div style={{
          ...
          border: '1px solid rgba(197,169,106,0.45)', borderRadius: '99px',
          padding: '5px 16px', marginBottom: '22px',
        }}>
          <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#C5A96A', flexShrink: 0 }} />
          <span style={{ fontSize: '10px', fontWeight: '600', color: '#C5A96A', letterSpacing: '0.18em', textTransform: 'uppercase' }}>
            Sistema de gestión
          </span>
        </div>
        <h1 style={{
          fontFamily: "'Cormorant Garamond', serif",
          fontSize: '46px', fontWeight: '600', color: '#ffffff',
          margin: '0 0 10px', lineHeight: 1.1, letterSpacing: '-0.01em',
        }}>
          Notaría Pública No. 120
        </h1>
        <p style={{ fontSize: '13px', fontWeight: '400', color: '#8A9BAD', letterSpacing: '0.06em', margin: 0 }}>
